...
Field Guide highlight (`GUI060102.get_TargetEmId`) → **F1** (or pad **R2**) search/join/depart. Join skips approval-required / password / full lobbies (toggleable in the REFramework UI). No target → error, no Arkveld fallback. **F1**/**R2** restarts while still searching; ignored once join starts (pad presses with no Field Guide target stay silent). **Esc** cancels. **F8**/**F9** start/stop the tracer.
